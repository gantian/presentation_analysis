function CreateDemoVideo(spk_ID)
    %% Tutorial
    ...
    videosTmp = dir(sprintf('%s/Videos/feedback_spk_%02d.mp4',currentPath,spk_ID));
    for iV = 1:length(videosTmp)
        ...
        audioSource = sprintf('W:/Research/Data/MM15_data/LabelData/speakerAudio/audio_Speaker_%02d.mp4',spk_ID);
        ...
        commandStrSeg = strcat(sprintf('ffmpeg'),...
            ...
            sprintf(' -i %s',audioSource),...  % audio source
            sprintf(' -i %s',videoName{1}),...  % audio source
            sprintf(' -i %s',videoName{2}),...
            sprintf(' -filter_complex "nullsrc=size=1920x1080 [base];'),...
            sprintf(' [1:v] setpts=PTS-STARTPTS, scale=1920x720 [down];'),...
            sprintf(' [2:v] setpts=PTS-STARTPTS, scale=1920x360 [up];'),...
